...
namespace Server.Message
{
    public class VesselMsgReader : ReaderBase
    {
        ...
        {
            if (!(message.Data is VesselBaseMsgData messageData))
                return;
            ...
            switch (messageData.VesselMessageType)
            {
                ...
                default:
                    LunaLog.Warning($"Unhandled vessel message type {messageData.VesselMessageType} from {client.PlayerName}");
                    break;
            }
        }
        ...
        private static void HandleVesselProto(ClientStructure client, VesselBaseMsgData message)
        {
            ...
            if (msgData.Data == null || msgData.NumBytes > msgData.Data.Length)
            {
                LunaLog.Warning($"Received invalid vessel proto data ({msgData.VesselId}) from {client.PlayerName}.");
                return;
            }
            ...
            VesselDataUpdater.RawConfigNodeInsertOrUpdate(
                msgData.VesselId,
                Encoding.UTF8.GetString(msgData.Data, 0, msgData.NumBytes));
        ...
        private static void HandleVesselsSync(ClientStructure client, VesselBaseMsgData message)
        {
            ...
            var safeVesselsCount = msgData.VesselIds == null
                ? 0
                : Math.Min(msgData.VesselsCount, msgData.VesselIds.Length);
            ...
            if (allVessels.Count > 0)
                LunaLog.Debug($"Sending {client.PlayerName} {allVessels.Count} vessels");
        }
        ...
        private static void HandleVesselCouple(ClientStructure client, VesselBaseMsgData message)
        {
            ...
            if (VesselContext.RemovedVessels.ContainsKey(msgData.CoupledVesselId))
                return;
